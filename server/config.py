...
redis_store = redis.Redis(host='mh.hicaiji.com', port=6379, password="hicaiji.com", db=3,
...
token_salt = "youguess"  # 服务端生成token的混淆加盐值
...
class Config:
    ...
    # flask-session配置
    SESSION_TYPE = "redis"
    SESSION_USE_SIGNER = True  # 对cookie中session_id进行隐藏处理 加密混淆 可以删了
    PERMANENT_SESSION_LIFETIME = 20  # session数据的有效期，单位秒 可以删了


# 开发环境
class DevelopmentConfig(Config):
    """开发模式的配置信息"""
    SQLALCHEMY_DATABASE_URI = 'mysql+pymysql://root:password@127.0.0.1:3306/flash_card?charset=utf8mb4'
    SESSION_REDIS = redis.Redis(host='127.0.0.1', port=6379, password="guess", db=2)  # 操作的redis配置
    DEBUG = True
...
class ProductionConfig(Config):
    """生产环境配置信息 jamkung环境"""
    SQLALCHEMY_DATABASE_URI = 'mysql+pymysql://root:password@127.0.0.1:3306/flash_card?charset=utf8mb4'
    SESSION_REDIS = redis.Redis(host='127.0.0.1', port=6379, password="guess...", db=3)  # 操作的redis配置
